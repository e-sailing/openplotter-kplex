...
import wx, sys, socket, time, webbrowser, os
...
class MyFrame(wx.Frame):
	...
	def __init__(self):
		...
		self.icon = wx.Icon(self.currentdir+"/data/kplex.png", wx.BITMAP_TYPE_PNG)
					...
					for i in nmea_list:
						if i[0] in [33,36]:
							...
							if exist==0:
								...
								self.list.SetItem(index, 3, dat)
					strsplit = (str(frase_nmea)[2:-5]).split('\\r\\n')
					for s in strsplit:
						frase_nmea_log+=s+'\n'
	...
	def sort(self, e):
		...
		for i in self.list_iter:
			self.list.InsertItem(index, i[0])
